...
def loss_and_gradients(x, y, params):
    ...
    W, b = params
    # YOU CODE HERE
    y_hat_encoded = classifier_output(x, params)
    y_hat_encoded += 1e-10
    y_encoded = np.zeros(y_hat_encoded.shape)
    y_encoded[y] = 1
    loss = -np.log(y_hat_encoded[y])
    gL = y_hat_encoded - y_encoded
    gW = np.dot(np.array(x).reshape(-1, 1), gL.reshape(1, -1))
    gb = gL * 1
    return loss, [gW, gb]
